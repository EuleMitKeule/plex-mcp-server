...
class PlexSessionsTools(PlexMcpTools):
    """Tools for managing Plex sessions and playback history."""
    ...
    def __init__(
        ...
        """Initialize the Plex Sessions Tools."""
        tools: list[AnyFunction] = [
            self.sessions_get_active,
            self.sessions_get_media_playback_history,
        ]

        super().__init__(plex_url, plex_token, permissions, tools)

    async def sessions_get_active(self, unused: Optional[str] = None) -> str:
                ...
                if len(results) > 1:
                    ...
                    for item in results:
                        ...
                        if item.type == "episode":
                            ...
                            item_info["season_number"] = getattr(
                                item, "parentIndex", "?"
                            )
