"""
Client-related tools for Plex Media Server.
...
class PlexClientTools(PlexMcpTools):
    """Tools for managing Plex clients and controlling playback."""
    ...
    def __init__(
        ...
        """Initialize the Plex Client Tools."""
        tools: list[AnyFunction] = [
            self.client_list,
            self.client_get_details,
            self.client_get_timelines,
            self.client_get_active,
        ]
        if permissions in [PermissionsType.WRITE, PermissionsType.DELETE]:
            tools.extend(
                [
                    self.client_start_playback,
                    self.client_control_playback,
                    self.client_navigate,
                    self.client_set_streams,
                ]
            )

        super().__init__(plex_url, plex_token, permissions, tools)

    async def client_list(self, include_details: bool = True) -> str:
    ...
    async def client_get_details(self, client_name: str) -> str:
            ...
            try:
                ...
                if matching_clients:
                    ...
                    matching_session_clients = [
                        ...
                        if hasattr(c, "title")
                        and client_name.lower() in c.title.lower()
                    ]
    ...
    async def client_get_timelines(self, client_name: str) -> str:
                ...
                if timeline is None:
                    ...
                    for session in sessions:
                        ...
                        if (
                            ...
                            and session.player.machineIdentifier
    ...
    async def client_get_active(self) -> str:
        ...
        try:
            ...
            for session in sessions:
                ...
                if hasattr(session, "player") and session.player:
                    ...
                    transcoding = False
                    if (
                        hasattr(session, "transcodeSessions")
                        and session.transcodeSessions
                    ):
                        transcoding = True
    ...
    async def client_start_playback(
        ...
        try:
            ...
            try:
                ...
                matching_clients = [
                    c for c in plex.clients() if client_name.lower() in c.title.lower()
                ]
                if matching_clients:
                    client = matching_clients[0]
                else:
        ...
        try:
            ...
            try:
                ...
                # Volume controls
                    ...
                    if parameter is None or parameter < 0 or parameter > 100:
                        return json.dumps(
                            {
                                "status": "error",
    ...
    async def client_navigate(self, client_name: str, action: str) -> str:
    ...
    async def client_set_streams(
        ...
        subtitle_stream_id: Optional[str] = None,
