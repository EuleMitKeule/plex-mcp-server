...
class PlexUserTools(PlexMcpTools):
    """Tools for managing Plex users and user statistics."""
    ...
    def __init__(
        ...
        """Initialize the Plex User Tools."""
        tools: list[AnyFunction] = [
            self.user_search_users,
            self.user_get_info,
            self.user_get_on_deck,
            self.user_get_watch_history,
            self.user_get_statistics,
        ]

        super().__init__(plex_url, plex_token, permissions, tools)
        self._plex_username = plex_username or ""

    async def user_search_users(self, search_term: Optional[str] = None) -> str:
    ...
    async def user_get_info(self, username: Optional[str]) -> str:
        ...
        """
        username = username or self._plex_username
        try:
            plex = self._plex_client.connection
    ...
    async def user_get_on_deck(self, username: Optional[str]) -> str:
        ...
        try:
            ...
            for stat in stats_list:
                ...
                media_type = media_type_map.get(
                    metadata_type, f"unknown-{metadata_type}"
                )
                ...
                user_stats[account_id]["media_types"][media_type]["duration"] += (
                    duration
                )
                ...
                if device_id is not None:
                    ...
                    user_stats[account_id]["devices"][device_name]["duration"] += (
                        duration
                    )
