...
class PlexServerTools(PlexMcpTools):
    """Tools for managing Plex server operations."""
    ...
    def __init__(
        ...
        """Initialize the Plex Server Tools."""
        tools: list[AnyFunction] = [
            self.server_get_info,
            self.server_get_bandwidth,
            self.server_get_current_resources,
            self.server_get_butler_tasks,
            self.server_get_alerts,
            self.server_get_plex_logs,
        ]
        if permissions in [PermissionsType.WRITE, PermissionsType.DELETE]:
            tools.extend(
                [
                    self.server_run_butler_task,
                ]
            )

        super().__init__(plex_url, plex_token, permissions, tools)

    def _extract_log_from_zip(self, zip_ref, log_file_name):
    ...
    async def server_get_plex_logs(
        self, num_lines: int = 100, log_type: str = "server"
    ) -> str:
        ...
        try:
            ...
            if (
                ...
                with zipfile.ZipFile(logs_path_or_data, "r") as zip_ref:
                    log_content = self._extract_log_from_zip(zip_ref, log_file_name)
            ...
            log_lines = (
                log_lines[-num_lines:] if len(log_lines) > num_lines else log_lines
            )
    ...
    async def server_get_info(self) -> str:
        ...
        try:
            ...
            server_info = {
                ...
                "certificate": plex.certificate
                if hasattr(plex, "certificate")
                else None,
                "sync": plex.sync if hasattr(plex, "sync") else None,
                "transcoder_active_video_sessions": plex.transcoderActiveVideoSessions,
                "transcoder_audio": plex.transcoderAudio
                if hasattr(plex, "transcoderAudio")
                else None,
                ...
                "owner_features": plex.ownerFeatures
                if hasattr(plex, "ownerFeatures")
                else None,
            }
    ...
    async def server_get_bandwidth(
        self, timespan: Optional[str] = None, lan: Optional[str] = None
    ) -> str:
            ...
            if hasattr(plex, "bandwidth"):
                ...
                for bandwidth in bandwidth_data:
                    ...
                    stats = {
                        ...
                        "device_id": bandwidth.deviceID
                        if hasattr(bandwidth, "deviceID")
                        else None,
                        ...
                        if hasattr(bandwidth, "bytes")
                        else None,
                        "is_local": bandwidth.lan
                        if hasattr(bandwidth, "lan")
                        else None,
    ...
    async def server_get_current_resources(self) -> str:
        ...
        try:
            ...
            if hasattr(plex, "resources"):
                ...
                for resource in server_resources:
                    ...
                    resource_entry = {
                        "timestamp": str(resource.at)
                        if hasattr(resource, "at")
                        else None,
    ...
    async def server_get_butler_tasks(self) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    return json.dumps(
                        {"status": "success", "data": butler_tasks}, indent=4
                    )
    ...
    async def server_get_alerts(self, timeout: int = 15) -> str:
    ...
    async def server_run_butler_task(self, task_name: str) -> str:
            ...
            if response.status_code in [200, 201, 202, 204]:
                ...
                if "<html>" in response.text:
                    ...
                    if title_match and title_match.group(1):
                        error_message = (
                            f"Failed to run butler task: {title_match.group(1)}"
                        )
                    ...
                    if h1_match and h1_match.group(1):
                        error_message = (
                            f"Failed to run butler task: {h1_match.group(1)}"
                        )

                return json.dumps(
                    {"status": "error", "message": error_message}, indent=4
                )
