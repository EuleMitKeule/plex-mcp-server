...
class PlexLibraryTools(PlexMcpTools):
    """Tools for managing Plex libraries."""
    ...
    def __init__(
        ...
        """Initialize the Plex Library Tools."""
        tools: list[AnyFunction] = [
            self.library_list,
            self.library_get_stats,
            self.library_get_details,
            self.library_get_recently_added,
            self.library_get_contents,
        ]
    ...
    async def library_list(self) -> str:
        """List all available libraries on the Plex server."""
        try:
            library: Library = self._plex_client.connection.library
            sections: list[LibrarySection] = library.sections()

            if not sections:
                return json.dumps(
                    {"message": "No libraries found on your Plex server."}
                )
            ...
            return json.dumps(
                {
                    section.title: {
                        ...
                        if (updated_at := section.updatedAt)
                        else None,
                    }
    ...
    async def library_get_stats(self, library_name: str) -> str:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                unwatched_url = urljoin(
                    base_url, f"library/sections/{section_id}/all?unwatched=1"
                )

                # Make concurrent requests for all and unwatched items
                all_data, unwatched_data = await asyncio.gather(
                    self._async_get_json(session, all_items_url, headers),
                    self._async_get_json(session, unwatched_url, headers),
                )
                ...
                if library_type == "movie":
                    ...
                    for movie in all_data.get("Metadata", []):
                        ...
                        for director in movie.get("Director", []):
                            ...
                            directors[director_name] = (
                                directors.get(director_name, 0) + 1
                            )
                    ...
                    if studios:
                        movie_stats["topStudios"] = dict(
                            sorted(studios.items(), key=lambda x: x[1], reverse=True)[
                                :5
                            ]
                        )
                    if decades:
                    ...
                    # Prepare URLs for concurrent requests
                    seasons_url = urljoin(
                        base_url, f"library/sections/{section_id}/all?type=3"
                    )
                    episodes_url = urljoin(
                        base_url, f"library/sections/{section_id}/all?type=4"
                    )

                    # Make concurrent requests for seasons and episodes
                    seasons_data, episodes_data = await asyncio.gather(
                        self._async_get_json(session, seasons_url, headers),
                        self._async_get_json(session, episodes_url, headers),
                    )
                    ...
                    for artist in all_data.get("Metadata", []):
                        ...
                        artist_tracks_data = await self._async_get_json(
                        ...
                        if (
                            ...
                            for track in artist_tracks_data["MediaContainer"][
                                "Metadata"
                            ]:
                                # Count total tracks
                    ...
                    if top_albums:
                        artist_stats["topAlbums"] = dict(
                            sorted(
                                top_albums.items(), key=lambda x: x[1], reverse=True
                            )[:10]
                        )
                    if all_years:
    ...
    async def library_refresh(self, library_name: str | None = None) -> str:
        ...
        try:
            if library_name:
                ...
                section = None
                all_sections = self._plex_client.connection.library.sections()
                ...
                self._plex_client.connection.library.refresh()
                return json.dumps(
                    {
                        "success": True,
    ...
    async def library_scan(self, library_name: str, path: str | None = None) -> str:
    ...
    async def library_get_details(self, library_name: str) -> str:
        ...
        try:
            ...
            if "scannerSettings" in data:
                ...
                for setting in data["scannerSettings"]:
                    if "value" in setting:
                        scanner_settings[setting.get("key", "unknown")] = setting[
                            "value"
                        ]
                if scanner_settings:
            ...
            if "advancedSettings" in data:
                ...
                for setting in data["advancedSettings"]:
                    if "value" in setting:
                        advanced_settings[setting.get("key", "unknown")] = setting[
                            "value"
                        ]
    ...
    async def library_get_recently_added(
        self, count: int = 50, library_name: str | None = None
    ) -> str:
        ...
        try:
            ...
            if library_name:
                ...
                recent = self._plex_client.connection.library.recentlyAdded()
                # Sort by date added (newest first) and limit to count
                if recent:
                    try:
                        recent = sorted(
                            recent,
                            key=lambda x: getattr(x, "addedAt", float("-inf"))
                            or float("-inf"),
                            reverse=True,
                        )[:count]
                    except Exception:
                ...
                try:
                    ...
                    if item_type == "movie":
                        ...
                        result["items"][item_type].append(
                            {
                                "artist": getattr(
                                    item, "parentTitle", "Unknown Artist"
                                ),
                                "title": item.title,
                                "addedAt": added_at,
                            }
            ...
            return json.dumps(
                {"error": f"Error getting recently added items: {str(e)}"}
            )

    async def library_get_contents(self, library_name: str) -> str:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                # Get all items
                all_items_url = urljoin(base_url, f"library/sections/{section_id}/all")
                all_data = await self._async_get_json(session, all_items_url, headers)
                all_data = all_data["MediaContainer"]
                ...
                if library_type == "movie":
                    ...
                    show_urls = [
                        (
                            item["ratingKey"],
                            urljoin(base_url, f"library/metadata/{item['ratingKey']}"),
                        )
                        for item in all_data.get("Metadata", [])
                    ]
                    show_responses = await asyncio.gather(
                        *[
                            self._async_get_json(session, url, headers)
                            for _, url in show_urls
                        ]
                    )
                    ...
                    for artist in all_data.get("Metadata", []):
                        ...
                        artists_info[artist_name]["viewCount"] = (
                            track_view_count
                            if track_view_count > 0
                            else orig_view_count
                        )
                        artists_info[artist_name]["skipCount"] = (
                            track_skip_count
                            if track_skip_count > 0
                            else orig_skip_count
                        )
